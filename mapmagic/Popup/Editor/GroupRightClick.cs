...
namespace MapMagic.Nodes.GUI
{
	public static class GroupRightClick
	{
			...
			public static Item GroupItems (Vector2 mousePos, Group grp, Graph graph, int priority=3)
			{
				...
				genItems.subItems.Add(CreateGroupItem(mousePos, graph));
				genItems.subItems.Add(GroupSelectedItem(mousePos, graph));
				genItems.subItems.Add(UngroupItem(grp, graph));
				genItems.subItems.Add(RemoveGroupItem(grp, graph));

				return genItems;
			}

			public static Item CopyItem (Group grp, Graph graph, bool extendedName=false)
			{
				string name = "Copy";
				if (extendedName) name += " group";
				...
				item.disabled = grp == null;
				item.onClick = () => GeneratorRightClick.copiedGenerators = graph.Export(grp);
				item.closeOnClick = true;
				return item;
			}

			public static Item CutItem (Group grp, Graph graph, bool extendedName=false)
			{
				string name = "Cut";
				if (extendedName) name += " group";
				...
				item.onClick = () =>
				{
					GeneratorRightClick.copiedGenerators = graph.Export(grp);
					RemoveGroup(grp, graph, withContent:true);
				};
				item.closeOnClick = true;
				return item;
			}

			public static Item CreateGroupItem (Vector2 mousePos, Graph graph)
			{
				return new Item("Create", onDraw: RightClick.DrawItem, priority: 12)
				{
			...
			public static Item GroupSelectedItem (Vector2 mousePos, Graph graph)
			{
				return new Item("Group selected", onDraw: RightClick.DrawItem, priority: 12)
				{
			...
			public static Item UngroupItem (Group grp, Graph graph)
			{
				return new Item("Ungroup", onDraw: RightClick.DrawItem, priority: 5)
				{
			...
			public static Item RemoveGroupItem (Group grp, Graph graph, bool extendedName=false)
			{
				string name = extendedName ? "Remove" : "Remove group";
				return new Item("Remove", onDraw: RightClick.DrawItem, priority: 4)
				{
