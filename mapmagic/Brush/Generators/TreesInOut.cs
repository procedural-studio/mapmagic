...
namespace MapMagic.Brush
{
	...
	public static class TreesOps
	{
		public static void TreesFromTerrain (Terrain terrain, TransitionsList tlist, Vector2D worldPos, Vector2D worldSize, HashSet<int> onlyIndexes=null)
		{
			...
			Vector3D terrainPos = terrain.transform.position.Vector3D();
			Vector3D terrainSize = terrain.terrainData.size.Vector3D();
			Vector2D relPos = worldPos-terrainPos.Vector2D() / terrainSize.Vector2D();
			Vector2D relSize = worldSize / terrainSize.Vector2D();
			...
			for (int i=0; i<allTrees.Length; i++)
			{
				...
				TreeInstance tree = allTrees[i]; //TODO: we baking it all to tree instance and then reading back?
				Vector3D treePosition = new Vector3D(tree.position.x*terrainSize.x, tree.position.y*terrainSize.y, tree.position.z*terrainSize.z) + terrainPos;
				Vector4D treeRotation = Quaternion.Euler(0, tree.rotation, 0).Vector4D();
				Vector3D treeScale = new Vector3D(tree.widthScale, tree.heightScale, tree.widthScale);
				Transition trs = new Transition(treePosition, treeRotation, treeScale);
				trs.num = tree.prototypeIndex;
				trs.instanceId = prototype.prefab.transform.GetInstanceID();
				tlist.Add(trs);
		...
		/// Works within brush only (won't add it to other trees)
		{
			...
			for (int t=0; t<trnList.count; t++)
			{
				...
				Transform prefab = ObjectsOps.IdToInstance(trn.instanceId);
				if (prefab == null)
					continue;
