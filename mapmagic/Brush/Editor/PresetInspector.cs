						...
						if (preset.guiShowUsedAutomatic)
						{
							...
							//like overridden parameters, but displaying only automatic
							{
								...
								if (automaticNames.ContainsKey(name)) //it's either radius, hardness, etc
									using (Cell.LineStd)
									{
										obj = Draw.UniversalField(obj, obj.GetType(), name); //using obj.GetType instead of 'mode' otherwise UniversalField will write an exception that it could not convert val (object) to Vector2D
