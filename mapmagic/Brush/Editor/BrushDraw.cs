...
namespace MapMagic.Brush
{
	public partial class BrushInspector
	{
		...
		static private PolyLineMesh polyLine = new PolyLineMesh(numCorners);
		static private PolyLineMesh traceLine = new PolyLineMesh(numCorners);
		static private PolyLineMesh dashLine = new PolyLineMesh(numCorners);
		...
		public static void DrawBrush (MapMagicBrush brush) 
		{
			...
			if (brush.tuneStroke.stamps.Length != 0)
			{
				...
				DrawCircle(polyLine, pos.Vector3(), radius, brush.mainColor, brush.lineThickness, corners, brush.terrains);
			}
		}


		private static void DrawTrace (PolyLineMesh line, IList<Vector2D> points, Color color, Terrain[] terrains, int startNum=0, int endNum=-1)
		{
			if (points.Count <= 1)
				return;
			...
			for(int i=0; i<flooredPoints.Length; i++)
				flooredPoints[i] = points[i].Vector3();

			FloorPoints(flooredPoints, terrains);

			line.DrawLine(flooredPoints, color, 8, zMode:PolyLineMesh.ZMode.Overlay, offset:0.1f);
		}


		private static void DrawDashLine (PolyLineMesh line, Vector2D start, Vector2D end, Color color, float dashSize, Terrain[] terrains)
		{
			...
			Vector3[] flooredStartEnd = new Vector3[] { start.Vector3(), end.Vector3() };
			...
			line.DrawLine(dashes, color, 8, zMode:PolyLineMesh.ZMode.Overlay, offset:0.1f);
		}


		private static float DrawCircle (PolyLineMesh line, Vector3 center, float radius, Color color, float thickness, Vector3[] corners, Terrain[] terrains)
		...
		/// Copy of lock circle, not reference since I'd like to be free to change it
		{
			...
			line.DrawLine(corners, color, thickness, zMode:PolyLineMesh.ZMode.Overlay, offset:0.1f);
