...
namespace MapMagic.Brush
{
	...
	public class TerrainCache
	{
		...
		public void UpdateCache ()
		{
			...
			worldPos = terrain.transform.position.Vector3D();
			worldSize = terrainData.size.Vector3D();
			...
			/// Copies pixels within given applyRect from heights matrix to terrain on Apply Changes
			{
				...
				if (splatsApply == null  ||  splatsApply.Count != scaledTexMaps.Count ||
					...
					splatsApply.rect.size.z < splatsApplyRect.size.z  ||  splatsApply.rect.size.z > splatsApplyRect.size.z+3)
						splatsApply = new MatrixSet(splatsApplyRect, Vector3D.zero, Vector3D.zero);
				...
				MatrixSet.CopyResized(src:scaledTexMaps, dst:splatsApply,
					srcRectPos: splatsApply.rect.offset.Vector2D() / scaleRatio + scaledTexMaps.rect.offset.Vector2D(), //splatsApplyRect is relative to terrain 0, so returning back offset since it's present in scaledTexMaps
					srcRectSize: splatsApply.rect.size.Vector2D() / scaleRatio,
				...
				{
					Vector2D relativeOffset = relApplyRect.offset.Vector2D() / heightRes;
		...
		#region Grass
			...
			/// Copies pixels within given applyRect from heights matrix to terrain on Apply Changes
			{
				...
				if (terrainData.detailPrototypes.Length==0)  //no grass of any mode added	
				{
				...
				if (grassApply == null  ||  grassApply.Count != scaledGrassMaps.Count ||
					...
					grassApply.rect.size.z < grassApplyRect.size.z  ||  grassApply.rect.size.z > grassApplyRect.size.z+3)
						grassApply = new MatrixSet(grassApplyRect, Vector3D.zero, Vector3D.zero);
				...
				MatrixSet.CopyResized(src:scaledGrassMaps, dst:grassApply,
					srcRectPos: grassApply.rect.offset.Vector2D() / scaleRatio + scaledGrassMaps.rect.offset.Vector2D(), //grassApplyRect is relative to terrain 0, so returning back offset since it's present in scaledGrassMaps
					srcRectSize: grassApply.rect.size.Vector2D() / scaleRatio,
