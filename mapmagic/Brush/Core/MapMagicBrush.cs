...
namespace MapMagic.Brush
{
	...
	public class MapMagicBrush : MonoBehaviour
	{
		...
		{
			Stamp stamp = new Stamp() {pos=pos.Vector2D(), radius=preset.radius, hardness=preset.hardness, margins=preset.margins};
			...
			//using (new Log.Timer("Override"))
			{
				...
				preset.ovd.SetIfContains("PrevPosition", typeof(Vector2D), trace.PrevStampPos.Vector2D());
			...
			using (Log.Group("Generate"))
			{
				...
				preset.graph.Generate(tileData, stop:stop);
