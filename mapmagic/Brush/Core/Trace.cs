...
namespace MapMagic.Brush
{
	...
	/// Periodically stamps the brush on terrain using it's spacing
	{
		...
		{
			Vector2D realDir = (LastPos.Vector2D() - LastStampPos.Vector2D()).Normalized; //next position based on current cursor
			Vector2D prevDir = stampPoses.Count >= 2 ?
				(LastStampPos.Vector2D() - stampPoses[stampPoses.Count-2].Vector2D()).Normalized :
				realDir;
...
//			return LastPos.Vector2D() + dir*(spacingDist-lastLength);
			return (new Vector2D());
		}
