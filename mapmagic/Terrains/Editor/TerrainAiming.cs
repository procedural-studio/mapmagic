...
namespace MapMagic.Terrains.GUI
{
	public static class TerrainAiming
	{
		...
		private static List<Coord> GetTilesInFrame (Rect screenFrame, Vector3 tileSize, Transform parent=null)
		{
			...
			CoordRect allTilesRect = new CoordRect( Coord.Floor(worldFrameCorners[0].Div(tileSize).Vector2D()), new Coord(0,0) );
			allTilesRect.Encapsulate( Coord.Floor(worldFrameCorners[1].Div(tileSize).Vector2D()) );
			allTilesRect.Encapsulate( Coord.Floor(worldFrameCorners[2].Div(tileSize).Vector2D()) );
			allTilesRect.Encapsulate( Coord.Floor(worldFrameCorners[3].Div(tileSize).Vector2D()) );
			allTilesRect.size.x ++;
			allTilesRect.size.z ++;
