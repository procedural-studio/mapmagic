...
/*		[Serializable] 
		public class ValTypeName
		{
			...
			public ValTypeName (Calculator.Vector val, Type mode, string name)
				{this.val = val; this.mode = mode; this.name = name;}
		}

		public SortedDictionary<ulong,ValTypeName> values = new SortedDictionary<ulong,ValTypeName>();*/
//TODO: unify and re-use
	...
	public class Override : ISerializationCallbackReceiver
	{
		[NonSerialized] private DictionaryOrdered<string,ObjType> dict = new DictionaryOrdered<string,ObjType>();
			//dictionary of structs - for each value mode is determined by is/GetType
		...
		/// Sets only if name key is in dict and mode match
		{
		...
		{
			// assigning mode of float for exposed channels
			Type assignType;
			if (entry.channel >= 0)
			{
				if (entry.mode == typeof(Coord))
					assignType = typeof(int);
				else
					assignType = typeof(float);
			}
			else
				assignType = entry.mode;
			...
			if (graph != null)
			{
				...
				//convert to vec and back to set channel and just to make sure it's of proper mode
		...
		/// Do not change values if they already exist
		{
			for (int i=dict.Count-1; i>=0; i--)
			{
				...
				if (!ovd.dict.Contains(name)  ||  ovd.dict[name].type!=type) //removing if improper mode too
					RemoveAt(i);
			}
		...
		#region Serialization
			...
			public void OnAfterDeserialize ()
			{
				if (serializedDict != null  &&  unityObjects != null)
					dict = (DictionaryOrdered<string,ObjType>)Den.Tools.Serialization.Serializer.Deserialize(serializedDict, unityObjects, Den.Tools.Serializer.FindNullType);
				...
				// If dict still could not be loaded due to serialization error - just creating new one
					dict = new DictionaryOrdered<string, ObjType>();
				#endif
			}
