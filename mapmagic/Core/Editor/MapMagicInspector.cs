...
namespace MapMagic.Core.GUI 
{
	...
	public class MapMagicInspector : Editor
	{
		...
		public int backgroundHeight = 0; //to draw mode background
		public int oldSelected = 0; //to repaint gui with new background if new mode was selected
			...
			using (Cell.LineStd)
				using (new Draw.FoldoutGroup(ref mapMagic.guiTerrainSettings, "Terrain Properties", isLeft:true))
					if (mapMagic.guiTerrainSettings)
					{
						...
						//copy
						Cell.EmptyLinePx(6);
						using (Cell.LineStd) Draw.ToggleLeft(ref mapMagic.terrainSettings.copyLayers, "Copy Layers to Terrains");
						using (Cell.LineStd) Draw.ToggleLeft(ref mapMagic.terrainSettings.copyTags, "Copy Tags to Terrains");
						using (Cell.LineStd) Draw.ToggleLeft(ref mapMagic.terrainSettings.copyStatic, "Copy Static Flags to Terrains");
