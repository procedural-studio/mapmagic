...
namespace MapMagic.Nodes
{
	...
	/// Not in objects but in Nodes since it's used by Brush core
	{
		...
		/// Floors object, and erases (yep) trs roation/scale values according to layer settings
		{
			...
			if (!useScale) trs.scale = new Vector3D(1,1,1);
			else if (scaleYonly) trs.scale = new Vector3D(1, trs.scale.y, 1);

			if (!useRotation) trs.rotation = Vector4D.QuaternionIdentity;
			else if (takeTerrainNormal) 
			{
				Vector3D terrainNormal = GetTerrainNormal(trs.pos.x, trs.pos.z, data.heights, data.globals.height, data.area.PixelSize.x);
				Vector3D terrainTangent = Vector3D.Cross(trs.rotation*new Vector3D(0,0,1), terrainNormal);
				trs.rotation = Vector4D.LookRotation(terrainTangent, terrainNormal);
			}
			else if (rotateYonly) trs.rotation = Vector4D.EulerToQuat(0,trs.Yaw,0);
		}


		public static Vector3D GetTerrainNormal (float fx, float fz, MatrixWorld heightmap, float heightFactor, float pixelSize)
		{
			Coord coord = heightmap.WorldToPixel(fx, fz);
			if (!heightmap.rect.Contains(coord, 1))
				return Vector3D.up;
			...
			return new Vector3D((prevXHeight-nextXHeight)*heightFactor, pixelSize*2, (prevZHeight-nextZHeight)*heightFactor).normalized;
		}
