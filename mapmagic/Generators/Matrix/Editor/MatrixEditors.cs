...
namespace MapMagic.Nodes.GUI
{
	public static class GeneratorEditors
	{
		...
		public static void DrawCurve (MatrixGenerators.Curve200 gen)
			{using(ProfilerExt.Profile("Curve200 Draw"))
		{
		...
		public static void DrawLevels (MatrixGenerators.Levels200 gen)
			{using(ProfilerExt.Profile("Levels200 Draw"))
		{
				...
				using (new Draw.FoldoutGroup(ref gen.guiParams, "Parameters", isLeft:true))
					if (gen.guiParams)
					{
						using (Cell.LineStd) { 
							...
							GeneratorDraw.DrawInletField(Cell.current, gen, typeof(Levels200), "inMin"); }
						using (Cell.LineStd) {
							Draw.Field(ref gen.gamma, "Gamma");
							GeneratorDraw.DrawInletField(Cell.current, gen, typeof(Levels200), "gamma"); }
						using (Cell.LineStd) {
							Draw.Field(ref gen.inMax, "In High");
							GeneratorDraw.DrawInletField(Cell.current, gen, typeof(Levels200), "inMax"); }

						Cell.EmptyLinePx(5);
												
						using (Cell.LineStd) {
							Draw.Field(ref gen.outMin, "Out Low");
							GeneratorDraw.DrawInletField(Cell.current, gen, typeof(Levels200), "outMin"); }
						using (Cell.LineStd) {
							Draw.Field(ref gen.outMax, "Out High");
							GeneratorDraw.DrawInletField(Cell.current, gen, typeof(Levels200), "outMax"); }
					}
			}
		}}
		...
		public static void DrawSelector (MatrixGenerators.Selector200 gen)
		{
			using (Cell.Padded(1,1,0,0)) 
			{
				...
				using (Cell.LineStd) 
				{
					...
					GeneratorDraw.DrawInletField(Cell.current, gen, typeof(Selector200), "units");
				}
				...
				if (gen.rangeDet == MatrixGenerators.Selector200.RangeDet.MinMax)
				{
					using (Cell.LineStd) 
					{
						...
						GeneratorDraw.DrawInletField(Cell.current, gen, typeof(Selector200), "from");
					}
					using (Cell.LineStd) 
					{
						Draw.Field(ref gen.to, "To");
						GeneratorDraw.DrawInletField(Cell.current, gen, typeof(Selector200), "to");
					}
				}
				else
				{
		...
		public static void CurveGeneratorEditor (MatrixGenerators.UnityCurve200 gen)
			{using(ProfilerExt.Profile("UnityCurve200 Draw"))
		{
			using (Cell.LinePx(GeneratorDraw.nodeWidth+4)) //don't really know why 4
				using (Cell.Padded(5))
				{
					Draw.AnimationCurve(gen.curve);
					GeneratorDraw.DrawInletField(Cell.current, gen, typeof(UnityCurve200), "curve");
				}
		}}

		[Draw.Editor(typeof(Blend200))]
		public static void BlendGeneratorEditor (Blend200 gen) => DrawLayers(gen, ref gen.layers, DrawBlendLayer);
		/*{
			...
			using (Cell.LinePx(20)) 
				LayersEditor.DrawLayersThemselves(Cell.Parent, gen.layers.Length, 
					onDraw: n => DrawTexturesLayer(gen, n), 
					roundBottom:true);

			using (Cell.LinePx(20)) 
				LayersEditor.DrawAddRemove(Cell.Parent, "Layers", ref gen.layers,
					onAdd: n => Layer.Create<Blend200.Layer>(gen),
					onRemove: n => GraphWindow.current.graph.Remove(gen.layers[n]),
					onMove: (f,t) => { graph.UnlinkInlet(newLayers[f]); graph.UnlinkInlet(newLayers[t]); } 
				);
		}*/
					...
					else
					{
						using (Cell.Row) 
						{
							...
							using (Cell.LineStd) 
							{
								...
								GeneratorDraw.DrawInletField(Cell.current, gen, typeof(Blend200.Layer), "opacity"); 
		...
		public static void NormalizeGeneratorEditor (Normalize200 gen) => DrawLayers(gen, ref gen.layers, DrawNormalizeLayer);
						...
						using (new Draw.FoldoutGroup(ref grassOut.guiAdvanced, "Advanced", padding:0))
							if (grassOut.guiAdvanced)
							{
								...
								if (GraphWindow.current.mapMagic != null  &&  GraphWindow.current.mapMagic is MapMagicObject mapMagicObject)
								{
									...
									if (mapMagicObject.globals.grassResDownscale < 1)
										mapMagicObject.globals.grassResDownscale = 1;
		...
		public static void DrawLayers<T> (Generator gen, ref T[] layers, Action<Generator,int> drawLayer) where T: Layer
		{
			Graph graph = GraphWindow.current.graph;
			T[] newLayers = layers;
			IMultiLayer multiLayer = gen as IMultiLayer;

			using (Cell.LinePx(20)) 
				LayersEditor.DrawLayersThemselves(Cell.Parent, layers.Length, 
					onDraw: n => drawLayer(gen, n), 
					roundBottom:true);

			using (Cell.LinePx(20)) 
				LayersEditor.DrawAddRemove(Cell.Parent, "Layers", ref newLayers,
					onAdd: n => Layer.Create<T>(gen),
					onRemove: n => GraphWindow.current.graph.Remove(newLayers[n]),
					onMove: (f,t) => 
					{ 
						if (newLayers[f] is IInlet<object> finlet)
							graph.UnlinkInlet(finlet); 
						if (newLayers[t] is IInlet<object> tinlet)	
							graph.UnlinkInlet(tinlet); 
					} 
				);
		...
		public static void TexturesGeneratorEditor (TexturesOutput200 gen) => DrawLayers(gen, ref gen.layers, DrawTexturesLayer);
		...
		private static void DrawTexturesLayer (Generator tgen, int num)
		{
			...
			using (Cell.LinePx(28))
			{
				...
				using (Cell.Row)
				{
					...
					Draw.EditableLabel(ref layer.guiName);
				}
				...
				using (Cell.Row)
				{
						...
						using (Cell.Padded(1,0,0,0))
					{
						...
						using (Cell.LineStd) 
						{
							...
							GeneratorDraw.DrawInletField(Cell.current, layer.Gen, typeof(TexturesOutput200.TextureLayer), "prototype");
						}

						if (layer.guiName == "Layer"  &&  layer.prototype != null)
							layer.guiName = layer.prototype.name;
					}
		...
		{ 
			...
			DrawLayers(gen, ref gen.layers, DrawCustomShaderLayer);
		}
		...
		public static void DrawDirectTexturesOutput (DirectTexturesOutput200 gen) => DrawLayers(gen, ref gen.layers, DrawDirectTexturesLayer);
		...
		public static void DrawDirectMapsOutput (DirectMatricesOutput200 gen) => DrawLayers(gen, ref gen.layers, DrawDirectMatricesLayer);
			...
			/*public static void DrawCustomMaterialWarning ()
			{
				...
				if (settings.materialType != Terrain.MaterialType.Custom)
				{
					using (Cell.LinePx(56))
					{
						...
						GUIStyle backStyle = UI.current.textures.GetElementStyle("DPUI/Backgrounds/FoldoutRight");
