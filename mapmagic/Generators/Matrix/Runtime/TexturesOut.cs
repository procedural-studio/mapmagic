...
using static MapMagic.Nodes.MatrixGenerators.TexturesOutput200;
using static MapMagic.Nodes.MatrixGenerators.CustomShaderOutput200;
using static MapMagic.Nodes.MatrixGenerators.DirectTexturesOutput200;

namespace MapMagic.Nodes.MatrixGenerators {
	[System.Serializable]
	public class BaseTextureLayer : Layer, IInlet<MatrixWorld>, IOutlet<MatrixWorld>
	{
...
//		public L[] layers = new L[0];
		public abstract IList<L> Layers { get; set; }

		public IList<IUnit> UnitLayers //TODO: plug while using layered generators
			{get{
				IUnit[] u = new IUnit[Layers.Count];
				for (int i=0; i<Layers.Count; i++) 
					u[i] = (IUnit)(Layers[i]);
				return u;
			}set{
				Layers = new L[value.Count];
				for (int i=0; i<value.Count; i++) 
					Layers[i] = (L)(object)(value[i]);
			}}
...
//		public virtual void SetLayers(object[] ls) => layers = Array.ConvertAll(ls, i=>(L)i);
		...
		/// But not sending to finalize
		{
			...
			MatrixWorld[] dstMatrices = new MatrixWorld[layers.Count];
			float[] opacities = new float[layers.Count];
			...
			for (int i=0; i<layers.Count; i++)
			{
				...
				else dstMatrices[i] = new MatrixWorld(data.area.full.rect, data.area.full.worldPos.Vector3D(), data.area.full.worldSize.Vector3D());
			...
			//normalizing
			if (stop!=null && stop.stop) return null;
			dstMatrices.FillNulls(() => new MatrixWorld(data.area.full.rect, data.area.full.worldPos.Vector3D(), data.area.full.worldSize.Vector3D()));
	...
	public class TexturesOutput200 : BaseTexturesOutput<TexturesOutput200.TextureLayer>
	{
		...
		public TextureLayer[] layers = new TextureLayer[0];
		public override IList<TextureLayer> Layers 
			{ get => layers; 
			set{
				layers = new TextureLayer[value.Count];
				for (int i=0; i<value.Count; i++) 
					layers[i] = (TextureLayer)(object)(value[i]);
			}}
	...
	public class CustomShaderOutput200 : BaseTexturesOutput<CustomShaderOutput200.CustomShaderLayer>
	{
		...
		public CustomShaderLayer[] layers = new CustomShaderLayer[0];
		public override IList<CustomShaderLayer> Layers 
			{ get => layers; 
			set{
				layers = new CustomShaderLayer[value.Count];
				for (int i=0; i<value.Count; i++) 
					layers[i] = (CustomShaderLayer)(object)(value[i]);
			}}
	...
	public class DirectTexturesOutput200 : BaseTexturesOutput<DirectTexturesOutput200.DirectTexturesLayer>
	{
		...
		public DirectTexturesLayer[] layers = new DirectTexturesLayer[0];
		public override IList<DirectTexturesLayer> Layers 
			{ get => layers; 
			set{
				layers = new DirectTexturesLayer[value.Count];
				for (int i=0; i<value.Count; i++) 
					layers[i] = (DirectTexturesLayer)(object)(value[i]);
			}}
		...
		public override void Generate (TileData data, StopToken stop) 
		{
			...
			{
				for (int i=0; i<layers.Length; i++)
					data.StoreOutput(layers[i], typeof(DirectTexturesOutput200), (layers[i].guiName, layers[i].channelNum, layers[i].Opacity), matrices[i]);
				data.MarkFinalize(Finalize, stop);
			}
			else 
				data.RemoveFinalize(finalizeAction);
		...
		public class ApplyData : IApplyData
		{
			...
			public virtual void Apply (Terrain terrain)
			{
				...
				holder.position = terrain.transform.position.Vector2D();
				holder.size = terrain.terrainData.size.Vector2D();
			}
	...
	public class DirectMatricesOutput200 : BaseTexturesOutput<DirectMatricesOutput200.DirectMatricesLayer>
	{
		...
		public DirectMatricesLayer[] layers = new DirectMatricesLayer[0];
		public override IList<DirectMatricesLayer> Layers 
			{ get => layers; 
			set{
				layers = new DirectMatricesLayer[value.Count];
				for (int i=0; i<value.Count; i++) 
					layers[i] = (DirectMatricesLayer)(object)(value[i]);
			}}
		...
		public override void Generate (TileData data, StopToken stop) 
		{
			...
			{
				for (int i=0; i<layers.Length; i++)
					data.StoreOutput(layers[i], typeof(DirectMatricesOutput200), layers[i].guiName, matrices[i]);
				data.MarkFinalize(Finalize, stop);
			}
			else 
				data.RemoveFinalize(finalizeAction);
		...
		public static void Finalize (TileData data, StopToken stop) 
		{
			...
			for (int m=0; m<matrices.Length; m++)
			{
				...
				matrixCopy.worldPos = data.area.active.worldPos.Vector3D();
				matrixCopy.worldSize = data.area.active.worldSize.Vector3D();
	...
	[GeneratorMenu(
		...
		iconName="GeneratorIcons/TexturesOut",
		helpLink = "https://gitlab.com/denispahunov/mapmagic/-/wikis/MatrixGenerators/Textures")]
	public class ExportOutput200 : OutputGenerator, IInlet<MatrixWorld>
	{
		public override (string, int) GetCodeFileLine () => GetCodeFileLineBase();  //to get here with right-click on generator

		[Val("Map", priority = 10, type = typeof(MatrixAsset))]	public MatrixAsset matrixAsset;

		public override OutputLevel OutputLevel => OutputLevel.Main;
		...
		public override void Generate (TileData data, StopToken stop) 
		{
			...
			if (enabled)
			{
				data.StoreOutput(this, typeof(ExportOutput200), matrixAsset, src);  //adding src since it's not changing
				data.MarkFinalize(Finalize, stop);
			}
			else 
				data.RemoveFinalize(finalizeAction);
		...
		public static void Finalize (TileData data, StopToken stop)
		{
			//preparing arrays
			if (stop!=null && stop.stop) return;
			data.GatherOutputs (typeof(ExportOutput200),
				out MatrixAsset[] assets,
				out MatrixWorld[] matrices,
				out MatrixWorld[] masks,
				inSubs:true);
			...
			//writing dict
			Dictionary<MatrixAsset,MatrixWorld> dict = new Dictionary<MatrixAsset,MatrixWorld>(); //we don't actually need dict, hashset will be enough
			for (int m=0; m<matrices.Length; m++)
			{
				if (matrices[m] == null)
					continue;

				//already in a dictionary for this asset -  adding with biome mask
				if (dict.ContainsKey(assets[m]))
				{
					assets[m].matrix.Add(matrices[m], masks[m]);
				}
				
				//matrix is not assigned yet
				else 
				{
					MatrixWorld matrixCopy = new MatrixWorld(matrices[m]); //to multyply with biome, and to keep it independent in case of matrix re-use feature
					
					if (masks[0] != null)
						matrixCopy.Multiply(masks[0]);
					...
					dict.Add(assets[m], matrixCopy);
					assets[m].matrix = matrixCopy;
				}
			}
		...
		public class ApplyData : IApplyData
		{
			...
			public Dictionary<MatrixAsset,MatrixWorld> dict;

			public virtual void Apply (Terrain terrain)
			{
				foreach (MatrixAsset asset in dict.Keys)
					asset.RefreshPreview();
			}
			...
			{get{
				if (dict.Count==0) return 0;
				else return dict.AnyValue().rect.size.x;
			}}
		}
