...
namespace MapMagic.Nodes.MatrixGenerators
{
	...
	public class Voronoi200 : Generator, IOutlet<MatrixWorld>
	{
		...
		{
			Vector3D matrixPos = matrix.worldPos;
			Vector3D matrixSize = matrix.worldSize;

			CoordRect rect = CoordRect.WorldToPixel(matrixPos.Vector2D(), matrixSize.Vector2D(), (Vector2D)cellSize);  //will convert to cells as well
			matrixPos = new Vector3D(rect.offset.x*cellSize, matrixPos.y, rect.offset.z*cellSize);  //modifying worldPos/size too
			matrixSize = new Vector3D(rect.size.x*cellSize, matrixSize.y, rect.size.z*cellSize);
...
/*
			...
			for (int x=min.x; x<max.x; x++)
			{
				...
				for (int z=min.z; z<max.z; z++)
				{
					...
					posMatrix.GetTwoClosest(worldPos.Vector3D(), out closest, out secondClosest, out minDist, out secondMinDist); 
...
*/
	...
	public class SimpleForm200 : Generator, IOutlet<MatrixWorld>, ISceneGizmo
	{
		...
		public override void Generate (TileData data, StopToken stop) 
		{
			...
			SimpleForm(matrix, offset.Vector2D(), ratScale * (Vector2D)data.area.active.worldSize, stop); //size is chunk-size relative
			matrix.Clamp01();
			data.StoreProduct(this, matrix);
		}
	...
	public class Import200 : Generator, IOutlet<MatrixWorld>, ISceneGizmo
	{
		...
		/// TODO: use CopyInterpolated
		{
			Coord min = dst.rect.Min; Coord max = dst.rect.Max;
			for (int x=min.x; x<max.x; x++)
				for (int z=min.z; z<max.z; z++)
				{
					Vector3 worldPos = dst.PixelToWorld(x,z).Vector3();
		...
		/// The new function, but doesn't work for some reason (no offset)
		{
			...
			Vector2D worldRatio = dst.worldSize.Vector2D() / src.worldSize.Vector2D();
			Vector2D rectRatio = dst.rect.size.Vector2D() / src.rect.size.Vector2D();

			Vector2D ratio = worldRatio / rectRatio;
			Vector2D readPos = (Vector2D)dst.rect.offset * ratio;
			if (compatibility) readPos -= src.worldPos.Vector2D() / src.PixelSize; //IDK why. Works properly _without_ this for clusters, but it will change the way Import works
		...
		/// Downsizes raw (src) and copies it to tile (dst)
		{
			...
			CoordRect dstPartRect = dst.WorldRectToPixels(src.worldPos.Vector2D(), src.worldSize.Vector2D());
	...
	/// Analog of Objects.Stroke, but made to make brush possible without objects
	{
		...
		{ 
			...
			matrix.Stroke(pixelPos, pixelRadius, hardness, 1);
	...
	public class WorldToPixelTest : Generator, IOutlet<MatrixWorld>, IPrepare
	{
		...
		public override void Generate (TileData data, StopToken stop) 
		{
			...
			if (interpolated) 
			{
				Vector3D vec = matrix.WorldToPixelInterpolated(pos.x, pos.z);
				Coord coord = Coord.Floor(vec.Vector2D());
				matrix[coord] = level;
				matrix[coord.x+1, coord.z] = level*0.75f;
				matrix[coord.x, coord.z+1] = level*0.5f;
				matrix[coord.x+1, coord.z+1] = level*0.25f;

				Vector3 retVec = matrix.PixelToWorld(vec.x, vec.z).Vector3();
			...
			else
			{
				...
				Vector3 retVec = matrix.PixelToWorld(coord.x, coord.z).Vector3();
				DebugGizmos.DrawDot("WorldToPixelTest", retVec, 6, Color.green);
			}
