...
namespace MapMagic.Nodes.SplinesGenerators
{
	...
	public class Interlink200 : Generator, IInlet<TransitionsList>, IOutlet<SplineSys>
	{
		...
		public override void Generate (TileData data, StopToken stop)
		{
			...
			PosTab posTab;
			if (withinTile)
				posTab = new PosTab(data.area.active.worldPos.Vector3D(), data.area.active.worldSize.Vector3D(), 16);
			else
			{
				Vector3D min = objs.Min();  min -= Vector3D.one;
				Vector3D max = objs.Max();  max += Vector3D.one;
			...
			if (stop != null && stop.stop) return;
			if (clamp == Clamp.Full) spline.Clamp(data.area.full.worldPos.Vector3(), data.area.full.worldSize.Vector3());
			if (clamp == Clamp.Active) spline.Clamp(data.area.active.worldPos.Vector3(), data.area.active.worldSize.Vector3());
		...
		public static void GabrielGraph ( PosTab objs, SplineSys splineSys, int maxLinks=4, int triesPerObj=8 )
		{
			...
			foreach (Transition trs in objs.All())
				idToPos.Add(trs.id, trs.pos.Vector3());
	...
	public class Pathfinding200 : Generator, IMultiInlet, IOutlet<SplineSys>
	{
		...
		[Val("Draft", "Inlet")]		public SplinesInlet draftIn = new SplinesInlet();
		...
		public override void Generate (TileData data, StopToken stop)
		{
			...
			clamped.Clamp(heights.worldPos.Vector3(), heights.worldSize.Vector3());
			...
			for (int l=0; l<src.lines.Length; l++)
			{
				...
				for (int s=0; s<line.segments.Length; s++)
				{
					...
					for (int i=0; i<pathCoord.Length; i++)
						pathWorld[i] = downsampledHeights.PixelToWorld(pathCoord[i].x, pathCoord[i].z).Vector3();
		...
		public void WeldEndpoints (SplineSys src)
		{
			...
			//writing endpoints back to lines
			for (int l=0; l<src.lines.Length; l++)
			{
				if (src.lines[l].segments.Length == 0)
					continue;
	...
	public class Align200 : Generator, IMultiInlet, IOutlet<MatrixWorld>
	{
		...
		[Val("Spline", "Inlet")]	public SplinesInlet splineIn = new SplinesInlet();
		[Val("Height", "Inlet")]	public MatrixInlet heightIn = new MatrixInlet();
	...
	{
		public class Layer : Nodes.Layer, IInlet<SplineSys> {}
		...
		public override void Generate (TileData data, StopToken stop) 
		{
			...
			for (int i = 0; i < layers.Length; i++)
			{
				...
				SplineSys otherSpline = data.ReadInletProduct(layer);
	...
	public class Avoid200 : Generator, IMultiInlet, IOutlet<SplineSys>
	{
		...
		public override void Generate (TileData data, StopToken stop)
		{
			...
			{
				points[o] = objs.arr[o].pos.Vector3();
	...
	public class Push200 : Generator, IMultiInlet, IOutlet<TransitionsList>
	{
		...
		[Val("Spline", "Objects")] public TransitionsInlet objsIn = new TransitionsInlet();
		[Val("Spline", "Spline")] public SplinesInlet splineIn = new SplinesInlet();
		...
		public override void Generate (TileData data, StopToken stop)
		{
			...
			for (int o=0; o<objs.count; o++)
				dst.arr[o].pos = points[o].Vector3D();
		...
		public override void Generate (TileData data, StopToken stop)
		{
			...
			foreach (Line line in src.lines)
			{
				...
				for (int p=0; p<points.Length; p++)
				{
					...
					if (rotate)
						trs.rotation = Quaternion.LookRotation(derivatives[p], Vector3.up).Vector4D();
	...
	public class Adjust2113 : Generator, IMultiInlet, IOutlet<SplineSys>
	{
		...
		[Val("Input", "Inlet")]		public SplinesInlet input = new SplinesInlet();
	...
	/*[System.Serializable]
	[GeneratorMenu (menu="Line", name ="Embankment", icon="GeneratorIcons/Constant", disengageable = true, helpLink ="https://gitlab.com/denispahunov/mapmagic/wikis/map_generators/constant")]
	public class EmbankmentGenerator : StandardGenerator, IOutlet<LineSys>
	{
		[Val("Line", "Inlet")]		public SplinesInlet splineIn = new SplineInlet();
		[Val("Height", "Inlet")]		public MatrixInlet heightIn = new MatrixInlet();
