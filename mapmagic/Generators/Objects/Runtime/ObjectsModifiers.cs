...
namespace MapMagic.Nodes.ObjectsGenerators
{
	...
	public class Adjust200 : Generator, IMultiInlet, IOutlet<TransitionsList>
	{
		...
		[Val("Intensity", "Inlet")]	public MatrixInlet intensityIn = new MatrixInlet();
		...
		public void Adjust (ref Transition trn, MatrixWorld intensityMatrix, Noise rnd)
		{
			...
			if (relativeness == Relativeness.relative)
			{
				...
				trn.pos += front.Vector3D() * frontRnd * intensity;
				trn.pos += right.Vector3D() * rightRnd * intensity;
				trn.pos.y += heightRnd * intensity; // / height; //not multiplying in output
				trn.Yaw = trn.Yaw  +  rotRnd * intensity;
			}
			else 
			{
				trn.scale = new Vector3D(1,1,1) * scaleRnd * intensity;
	...
	public class Mask200 : Generator, IMultiInlet, IOutlet<TransitionsList>
	{
		...
		[Val("Masked", "Inlet")]	public TransitionsInlet srcIn = new TransitionsInlet();
		[Val("Unmasked", "Inlet")]	public TransitionsInlet invIn = new TransitionsInlet();
		[Val("Mask", "Inlet")]	public MatrixInlet maskIn = new MatrixInlet(); 
	...
	/// Designed to use instead Move's intensity mask, but what is the purpose if it's done the same? However it still works
	{
		...
		[Val("Original", "Inlet")]	public TransitionsInlet srcIn = new TransitionsInlet();
		[Val("Modified", "Inlet")]	public TransitionsInlet modIn = new TransitionsInlet();
		[Val("Mask", "Inlet")]		public MatrixInlet maskIn = new MatrixInlet();
		...
		public void Lerp (TransitionsList list1, TransitionsList list2, TransitionsList dst, MatrixWorld mask)
		{
			...
			for (int t=0; t<list2.count; t++)
			{
				...
				trs1.rotation = Vector4D.Lerp(trs1.rotation, trs2.rotation, percent);
	...
	public class Floor200 : Generator, IMultiInlet, IOutlet<TransitionsList>
	{
		...
		[Val("Input", "Inlet")]		public TransitionsInlet srcIn = new TransitionsInlet();
		...
		[Val("Take Height Normal", isLeft=true)] public bool takeHeightNormal = false;
		...
		public override void Generate (TileData data, StopToken stop)
		{
			...
			for (int t=0; t<dst.count; t++)
			{
				...
				if (takeHeightNormal)
				{
					Vector3D normal = PositioningSettings.GetTerrainNormal(dst.arr[t].pos.x, dst.arr[t].pos.z, heights, heights.worldSize.y, heights.PixelSize.x);
					dst.arr[t].rotation = Vector4D.FromToRotation(Vector3D.up, normal);
				}
			}
	...
	public class Split200 : Generator, IInlet<TransitionsList>, IMultiLayer
	{
		...
		public class SplitLayer : Layer, IOutlet<TransitionsList>
		{
		...
		public IList<IUnit> UnitLayers { get => layers; set {return;} } //don't set anything
	...
	public class Rarefy200 : Generator, IInlet<TransitionsList>, IMultiInlet, IOutlet<TransitionsList>
	{
		...
		public override void Generate (TileData data, StopToken stop)
		{
			...
			PosTab srcTab = new PosTab(data.area.full.worldPos.Vector3D(), data.area.full.worldSize.Vector3D(), 16);
			srcTab.Add(src);
			...
			for (int i=0; i<subtrahends.Length; i++)
			{
				...
				subtrahends[i].posTab = new PosTab(data.area.full.worldPos.Vector3D(), data.area.full.worldSize.Vector3D(), 16);
	...
	public class Combine200 : Generator, IMultiInlet, IOutlet<TransitionsList>
	{
		...
		public TransitionsInlet[] inlets = new TransitionsInlet[2] { new TransitionsInlet(), new TransitionsInlet() };
		...
		private void Spread (Transition trn, TransitionsList spreadedList, Noise random)
		{
			...
			for (int n=0; n<num; n++)
			{
				...
				Transition newPos = new Transition() {
					pos = new Vector3D(posX, trn.pos.y, posZ),
	...
	{
		[Val(name="Objects", priority=2)]	public TransitionsInlet objectsIn = new TransitionsInlet();
		[Val(name="Canvas", priority=2)]	public MatrixInlet canvasIn = new MatrixInlet();  //TODO: canvas could be applied with blend
		[Val(name="Mask", priority=2)]		public MatrixInlet maskIn = new MatrixInlet();
	...
	public class Flatten200 : Generator, IMultiInlet, IOutlet<MatrixWorld>
	{
		...
		[Val("Positions", "Inlet")]	public TransitionsInlet positionsIn = new TransitionsInlet();
		[Val("Heights", "Inlet")]	public MatrixInlet heightsIn = new MatrixInlet();
		...
		{
			Vector2D mapCenter = matrix.WorldToPixelInterpolated(center.x, center.z).Vector2D();
	...
	public class Stamp200 : Generator, IMultiInlet, IOutlet<MatrixWorld>
	{
		...
		[Val("Positions", "Inlet")]		public TransitionsInlet positionsIn = new TransitionsInlet();
		[Val("Stamp", "Inlet")]			public MatrixInlet stampIn = new MatrixInlet();
		...
		private static void StampMatrix (MatrixWorld matrix, MatrixWorld stamp, CoordRect stampRect,
			Vector3D center, Vector4D rotation, float radius, float hardness, float intensity, bool blendAdditive,
			bool useRotation, bool useFalloff)
		{
			...
			if (useRotation)
				rotDirection = (rotation * new Vector3D(1,0,0)).Vector2D();
	...
	public class Forest200 : Generator, IMultiInlet, IOutlet<TransitionsList>
	{
		...
		public TransitionsInlet seedlingsIn = new TransitionsInlet() { guiName = "Seedlings" };  
		public TransitionsInlet otherTreesIn = new TransitionsInlet()  { guiName = "Other Trees" };
		public MatrixInlet soilIn = new MatrixInlet()   { guiName = "Soil" };
		...
		public override void Generate (TileData data, StopToken stop)
		{
			...
			TransitionsList forest = Forest(seedlings, otherTrees, soil, data.area.full.worldPos.Vector3D(), data.area.full.worldSize.Vector3D(), random, stop);
		...
		public TransitionsList Forest (TransitionsList seedlings, TransitionsList otherTrees, 
			MatrixWorld soil, Vector3D worldPos, Vector3D worldSize, Noise random, StopToken stop=null)
		{
			float cellSize = 1000 / Mathf.Sqrt(density);

return null;}
//TODO: waiting to transfer positionslist to native
/*
		...
		private void TransitionsToMatrix (PositionMatrix matrix, TransitionsList trns, bool scaleIsAge)
		{
			for (int t=0; t<trns.count; t++)
			{
				...
				if (currAge > prevAge)
					matrix.SetPosition( new Vector3D(trns.arr[t].pos.x, currAge, trns.arr[t].pos.z) );
			}
		}
...
*/
	...
	public class SlideGenerator200 : Generator, IMultiInlet, IOutlet<TransitionsList>
	{
		...
		[Val("Input", "Inlet")]		public TransitionsInlet srcIn = new TransitionsInlet();
		[Val("Stratum", "Inlet")]		public MatrixInlet stratumIn = new MatrixInlet();
	...
	public class Move210 : Generator, IInlet<TransitionsList>, IMultiInlet, IOutlet<TransitionsList>
	{
		...
		{ 
			...
			for (int t=0; t<dst.count; t++)
			{
				...
				dst.arr[t].pos = dst.arr[t].pos + (direction*percent).Vector3D();
			}
	...
	public class ShrinkScale210 : Generator, IInlet<TransitionsList>, IOutlet<TransitionsList>
	{
		...
		{ 
			...
			if (!invert)
				for (int t=0; t<dst.count; t++)
					dst.arr[t].scale = Vector3D.one + dst.arr[t].scale*multiply;
			else
				for (int t=0; t<dst.count; t++)
					dst.arr[t].scale = (dst.arr[t].scale - Vector3D.one)/multiply;
