...
namespace MapMagic.Locks
{
	public class ObjectsData : ILockData
	{
		...
		public void Read (Terrain terrain, Lock lk) 
		{
			...
			Vector2D mmPos = terrain.transform.parent.parent.position.Vector2D();

			center = lk.worldPos.Vector2D() + mmPos;
			...
			if (lockParent == null)
			{
				...
				for (int c=0; c<tileChildCount; c++)
				{
					...
					if (child.name == lockParentName || //name match
						(child.localPosition.Vector2D() == center  &&  child.name.StartsWith("LockedObject")))  //or position match, in case of renaming (no s at the end)
			...
			if (lockParent == null)
			{
				...
				go.transform.localPosition = center.Vector3();
			...
			for (int i=0; i<childCount; i++)
			{
				...
				lockedObjsPoses.Add(child.gameObject, child.position - mmPos.Vector3());
			}
			...
			if (pool != null)
			{
				for (int i=pool.transform.childCount-1; i>=0; i--)
				{
					...
					if (dist < (radius+transition)*(radius+transition))
					{
						...
						{
							lockedObjsPoses.Add(child.gameObject, child.transform.position - mmPos.Vector3());
		...
		public void WriteInThread (IApplyData applyData)
		{
			...
			for (int p=0; p<applyObjsData.transitions.Length; p++)
			{
				...
				{
					Vector3D pos = transitionList[t].pos;
					if (pos.x < min.x || pos.x > max.x ||
						pos.z < min.z || pos.z > max.z) continue;
		...
		public void ApplyHeightDelta (Matrix srcHeights, Matrix dstHeights)
		{
			...
			foreach (var kvp in lockedObjsPoses)
			{
				...
				Vector2D relPos = (pos.Vector2D()-center) / relRectSize;  //relative to lock circle
