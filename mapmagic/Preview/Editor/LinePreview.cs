...
{
	public class LinePreview : IPreview
	{
		[NonSerialized] public LineSys lineSys = null;  //to pass to thread
		[NonSerialized] public PolyLineMesh polyLine;  //in thread and in apply
		...
		#region Generate
			...
			{ 
				...
				this.lineSys = (LineSys)data.ReadOutletProduct(outlet); 
			...
			{
				if (lineSys == null) { Stage = PreviewStage.Blank; return; }

				//auto-line
				/*Vector3[][] nodes = new Vector3[lineSys.lines.Length][];
				for (int s=0; s<lineSys.lines.Length; s++)
					nodes[s] = lineSys.lines[s].nodes;

				if (polyLine == null) polyLine = new PolyLineMesh(0);
				polyLine.SetPointsThread(nodes);*/
				
				//line
				//Vector3[][] points = lineSys.GetAllPoints(resPerUnit:0.2f, minRes:3, maxRes:20);
				Vector3[][] points = new Vector3[lineSys.lines.Length][];

				for (int l=0; l<lineSys.lines.Length; l++)
				{
					Line line = lineSys.lines[l];
					points[l] = new Vector3[line.count];

					for (int p=0; p<line.count; p++)
					{
						Vector3D pos = line.nodes[p];
						pos -= worldPos;
			...
			private void PrepareNodesMeshArrays ()
			{
				...
				int nodesNum = 0;
				for (int s=0; s<lineSys.lines.Length; s++)
					nodesNum += lineSys.lines[s].nodes.Length;
				...
				int i = 0;
				for (int s=0; s<lineSys.lines.Length; s++)
					for (int n=0; n<lineSys.lines[s].nodes.Length; n++)
					{
						Vector3 pos = lineSys.lines[s].nodes[n];
				...
				int nodesNum = lineSys.NodeCount;
				...
				int i = 0;
				for (int l=0; l<lineSys.lines.Length; l++)
					for (int n=0; n<lineSys.lines[l].count; n++)
					{
						Vector3D pos = lineSys.lines[l].nodes[n];
						pos -= worldPos;
						Vector3 posv = new Vector3(pos.x/worldSize.x, pos.y/worldHeight, pos.z/worldSize.z);

						for (int v=0; v<4; v++)
							nodesMeshVerts[i*4+v] = posv;
						i++;
					}
		...
		#region Window
			...
			{
				LinePreviewWindow window = ScriptableObject.CreateInstance<LinePreviewWindow>();
			...
			public class LinePreviewWindow : BaseMatrixWindow, IPreviewWindow
			{
				public IPreview Preview { get{return preview;} set{preview = value as LinePreview;} }
				public LinePreview preview;
		...
		#region GUI
			...
			{
				if (lineSys == null  ||  polyLine == null  ||  polyLine.mesh == null) { Draw.Rect(PreviewDraw.BackgroundColor); return; }
