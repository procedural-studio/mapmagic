	...
	public class ObjectsPreview : IPreview
	{
		...
		#region Generate
			...
			public void SetObject (TransitionsList posTab, MatrixWorld heights, Area area)
			{
				...
				this.worldPos = area.active.worldPos.Vector3D();    
				this.worldSize = area.active.worldSize.Vector3D();
			...
			public void ExecuteInThread ()
			{
				...
				count = trns.CountInRect(worldPos.Vector2D(), worldSize.Vector2D());
				...
				{
					Vector3D pos = trns.arr[t].pos;
					pos -= worldPos;
					pos = new Vector3D (pos.x/worldSize.x, pos.y, pos.z/worldSize.z);
				//	if (heights != null) pos.y = pos.y/heights.worldSize.y;
					pos.y = 0;

					vertices[i] = vertices[i+1] = vertices[i+2] = vertices[i+3] = vertices[i+4] = vertices[i+5] = pos.Vector3();
